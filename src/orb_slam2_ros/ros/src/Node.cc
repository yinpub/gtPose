...
#include <iostream>
//冒号后面相当于将image_transport传入image_transport_构造函数，image_transport_=image_transport；
...
//      orb_slam_->SaveKeyFrameTrajectoryTUM("/home/host/zhongxiangrong/orb_slam_2_ros/KeyFrameTrajectoryTum.txt");
//      orb_slam_->SaveTrajectoryTUM("/home/host/zhongxiangrong/orb_slam_2_ros/TrajectoryTum.txt");
//  }
  delete orb_slam_;
}
...
void Node::Init () {
  ...
  node_handle_.param(name_of_node_ + "/save_trajectoryTum", save_trajectoryTum_param_, false);

   // Create a parameters object to pass to the Tracking system
   ORB_SLAM2::ORBParameters parameters;//构建一个ORB参数对象，包含了相机内参
   LoadOrbParameters (parameters);//载入参数

  orb_slam_ = new ORB_SLAM2::System(voc_file_name_param_, sensor_, parameters, map_file_name_param_, load_map_param_,this);
  ...
  if (publish_pose_param_) {
      ...
      pose_odom_publisher_ = node_handle_.advertise<nav_msgs::Odometry> (name_of_node_+"/pose_odom", 1);
...
void Node::PublishPositionAsPoseStamped (cv::Mat position) {
  ...
  // Make transform from camera frame(camera_link) to target frame(base_link)
  ...
  pose_odom_msg.pose.pose = pose_msg.pose;
  pose_odom_msg.header = pose_msg.header;
  pose_odom_msg.child_frame_id = camera_frame_id_param_;
  pose_odom_publisher_.publish(pose_odom_msg);
...
void Node::LoadOrbParameters (ORB_SLAM2::ORBParameters& parameters) {
  ...
  if (load_calibration_from_cam)
  {
    ...
    {
      parameters.fx = camera_info->K[0];//内参矩阵fx, 0 ,cx;0, fy ,cy;0, 0, 1
      ...
      return;//这里返回了就不用进行下面，从文件中读取畸变参数和内参了。
    }
  }

  bool got_cam_calibration = true;
  //&=是按位与，如果后面的getParam获取参数成功，则返回true，和got_cam_calibration做“与”操作以后赋值给got_cam_calibration
  //相当于判断是否全部都获取成功。
  if (sensor_== ORB_SLAM2::System::STEREO || sensor_==ORB_SLAM2::System::RGBD)
  {
