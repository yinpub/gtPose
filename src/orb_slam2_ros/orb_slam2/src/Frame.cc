     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    mfScaleFactor = mpORBextractorLeft->GetScaleFactor();  //获取设置的缩放因子
    mfLogScaleFactor = log(mfScaleFactor);//缩放因子的自然对数
    mvScaleFactors = mpORBextractorLeft->GetScaleFactors();//每层相对第0层的缩放因子的向量
    mvInvScaleFactors = mpORBextractorLeft->GetInverseScaleFactors();//上面那个向量的倒数
    mvLevelSigma2 = mpORBextractorLeft->GetScaleSigmaSquares();//sigma2就是每层图像相对初始图像缩放因子的平方
    mvInvLevelSigma2 = mpORBextractorLeft->GetInverseScaleSigmaSquares();//上面那个向量的倒数

    // ORB extraction
    //step2.对图像特征提取，第一个参数0-左，1-右
    ExtractORB(0,imGray);
