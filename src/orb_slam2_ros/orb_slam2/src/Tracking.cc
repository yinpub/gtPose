...
{
//tracking构造函数
Tracking::Tracking(System *pSys,                //系统实例对象
                   ...
                   ORBParameters& parameters):
                      mState(NO_IMAGES_YET),    //系统跟踪状态 SYSTEM_NOT_READY=-1,NO_IMAGES_YET=0,NOT_INITIALIZED=1,OK=2,LOST=3
                      mSensor(sensor),
                      mbOnlyTracking(false),    //仅定位模式，如果有地图load，则可以不建图，直接根据已有的地图进行定位。这里是SLAM模式
                      mbVO(false),              //当处于仅定位模式，这个bool形表示了跟踪好坏。
                      mpORBVocabulary(pVoc),
                      mpKeyFrameDB(pKFDB),
                      mpInitializer(static_cast<Initializer*>(NULL)),//单目模式下的地图初始化，先设置为空指针。
                      mpSystem(pSys),
                      mpFrameDrawer(pFrameDrawer),
                      mpMap(pMap),
                      mnLastRelocFrameId(0),
                      mnMinimumKeyFrames(5)     //跟踪丢失中不会重置的最少关键帧数量。
{
    //Unpack all the parameters from the parameters struct (this replaces loading in a second configuration file)
    mMaxFrames = parameters.maxFrames;//即fps相机帧率
    mbRGB = parameters.RGB;             //0为BGR，1为RGB
    mThDepth = parameters.thDepth;
    nFeatures = parameters.nFeatures;   //每一帧提取的特征点数1500
    fScaleFactor = parameters.scaleFactor;//图像金字塔的变化尺度1.2
    nLevels = parameters.nLevels;           //金字塔层数 8
    fIniThFAST = parameters.iniThFAST;      //提取fast特征点的默认阈值 20
    fMinThFAST = parameters.minThFAST;      //如果上面阈值提取不出足够的特征点，就用最小阈值 8
    ...
    mbf = parameters.baseline;//双目的基线

    // Max/Min Frames to insert keyframes and to check relocalization最大/最小帧数插入关键帧用来重定位
    mMinFrames = 0;
    //mMaxFrames = fps;
    ...
    //单目初始化时候用mpIniORBextractor作为特征提取器，两倍的特征点
    ...
    {
        //判断一个3D点远近的阈值 mbf*mThDepth/fx
        mThDepth = mbf*(float)mThDepth/parameters.fx;
        cout << endl << "Depth Threshold (Close/Far Points): " << mThDepth << endl;
    }

    //设置深度因子，将深得像素值转为实际深度，输入mDepthMapFactor深度图像中对应1m的像素值
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    ...
    //step2.这里将深度相机的disparity转为真实尺度下的深度
    //mDepthMapFactor为对应1m的深度图像素
