...
System::System(const string strVocFile, const eSensor sensor, ORBParameters& parameters,
               const std::string& map_file, bool load_map,Node* node): // map serialization addition
               ...
               map_file(map_file), load_map(load_map)
{
    ...
    mpVocabulary = new ORBVocabulary();//ORB_SLAM2::ORBVocabulary()
    ...
    //Create Drawers. These are used by the Viewer绘制帧
    mpFrameDrawer = new FrameDrawer(mpMap);

    //Initialize the Tracking thread
    //(it will live in the main thread of execution, the one that called this constructor)
    mpTracker = new Tracking(this, //指向system的指针
                             mpVocabulary, //类成员指针ORB字典
                             mpFrameDrawer,//类成员指针帧绘画器
                             mpMap, //类成员地图
                             mpKeyFrameDatabase, //类成员指针关键帧数据库
                             mSensor, //类成员sensor对象mSensor
                             parameters);//相机内参和相关ORB提取参数

    //Initialize the Local Mapping thread and launch

    mpLocalMapper = new LocalMapping(mpMap, mSensor==MONOCULAR,node);
...
void System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)
{
    ...
    //主要是判断SLAM模式还是仅定位模式
    {
        unique_lock<mutex> lock(mMutexMode);//独占锁，为了mbActivateLocalizationMode和mbDeactivateLocalizationMode不会发生混乱。
        if(mbActivateLocalizationMode)//如果为真，会关闭局部地图线程
        {
            mpLocalMapper->RequestStop();
