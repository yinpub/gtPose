...
public:
    ...
    // tree
    const int    NUM_EXCLUDE_RECENT = 30; //关键帧数量超过这个阈值，才会进行闭环检测 simply just keyframe gap (related with loopClosureFrequency in yaml), but node position distance-based exclusion is ok. 
